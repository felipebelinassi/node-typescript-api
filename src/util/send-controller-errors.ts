...
const handleClientErrors = (err: mongoose.Error.ValidationError): CustomError => {
  const duplicatedKindErrors = Object.values(err.errors)
    .filter(({ kind }) => kind === CustomValidation.DUPLICATED);
...
const sendCreateUpdateError = (res: Response, err: mongoose.Error.ValidationError | Error): void => {
